...
EMAIL_VERIFICATION_SEND_INTERVAL = os.environ.get(
    "EMAIL_VERIFICATION_SEND_INTERVAL", "1"  # TODO: 生产环境改回 60
)
try:
    EMAIL_VERIFICATION_SEND_INTERVAL = int(EMAIL_VERIFICATION_SEND_INTERVAL)
except ValueError:
    EMAIL_VERIFICATION_SEND_INTERVAL = 1
...
# 短信验证配置（梦网SMS）
####################################
# - ENABLE_SMS_VERIFICATION：是否启用短信验证（默认 False）
# - SMS_MWAPI_USER_ID：梦网API账号
# - SMS_MWAPI_PASSWORD：梦网API密码
# - SMS_MWAPI_MAIN_ADDR：梦网API主地址
# - SMS_MWAPI_BAK_ADDRS：梦网API备用地址（逗号分隔）
# - SMS_VERIFICATION_CODE_TTL：短信验证码有效期秒数（默认 600）
# - SMS_VERIFICATION_SEND_INTERVAL：同手机号再次发送的冷却秒数（默认 60）
# - SMS_VERIFICATION_MAX_ATTEMPTS：短信验证码最大验证次数（默认 5）

ENABLE_SMS_VERIFICATION = (
    os.environ.get("ENABLE_SMS_VERIFICATION", "False").lower() == "true"
)

# 梦网SMS API配置
SMS_MWAPI_USER_ID = os.environ.get("SMS_MWAPI_USER_ID", "")
SMS_MWAPI_PASSWORD = os.environ.get("SMS_MWAPI_PASSWORD", "")
SMS_MWAPI_MAIN_ADDR = os.environ.get("SMS_MWAPI_MAIN_ADDR", "")
SMS_MWAPI_BAK_ADDRS = os.environ.get("SMS_MWAPI_BAK_ADDRS", "")

# 短信验证码策略
SMS_VERIFICATION_CODE_TTL = os.environ.get("SMS_VERIFICATION_CODE_TTL", "600")
try:
    SMS_VERIFICATION_CODE_TTL = int(SMS_VERIFICATION_CODE_TTL)
except ValueError:
    SMS_VERIFICATION_CODE_TTL = 600

# 开发阶段设为 1 秒
SMS_VERIFICATION_SEND_INTERVAL = os.environ.get("SMS_VERIFICATION_SEND_INTERVAL", "1")  # TODO: 生产环境改回 60
try:
    SMS_VERIFICATION_SEND_INTERVAL = int(SMS_VERIFICATION_SEND_INTERVAL)
except ValueError:
    SMS_VERIFICATION_SEND_INTERVAL = 1

SMS_VERIFICATION_MAX_ATTEMPTS = os.environ.get("SMS_VERIFICATION_MAX_ATTEMPTS", "5")
try:
    SMS_VERIFICATION_MAX_ATTEMPTS = int(SMS_VERIFICATION_MAX_ATTEMPTS)
except ValueError:

...
<div class={outerClassName}>
	...
	<input
		{id}
		class={`${inputClassName} ${show ? '' : 'text-security-hidden'} ${($settings?.highContrastMode ?? false) ? 'placeholder:text-gray-700 dark:placeholder:text-gray-100' : ' outline-hidden placeholder:text-gray-300 dark:placeholder:text-gray-600'}`}
		{placeholder}
		type="text"
		{value}

...
export const getGlobalAPIConfig = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/global_api`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const setGlobalAPIConfig = async (token: string, config: object) => {
	let error = null;
